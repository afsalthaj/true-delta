...
trait DeltaSyntax {
  ...
  implicit class DeltaOpListSyntax[A : HasId](a: List[DeltaOp[A]]) {
    ...
    def asMeta(implicit P: ParentName[A]): Meta  = a.flatMap {
      case Create(a1) => Meta(ParentName[A].name(a1), Ior.Right(a1.toString), Nil)
      case Delete(a1) => Meta(ParentName[A].name(a1), Ior.Left(a1.toString), Nil)
      case Update(a1, a2, meta) => Meta(ParentName[A].name(a2), Ior.Both(a1.toString, a2.toString), meta)
